...
const RULES = [
  ...
  // Lookaround keeps this off Obsidian wikilinks — [[Add Surprise Sparingly]] is a note, not a placeholder.
  ['text-placeholder',  'provenance', 2, /(?<!\[)\[(?:Your|Insert|Add|Company|Name of)\b[^\]]{0,40}\](?!\])|\((?:Add|Insert) your [^)]{0,40}\)/i],
...
function walk(p, acc = []) {
  ...
  for (const e of readdirSync(p)) {
    if (['.git', 'node_modules', '.obsidian', '.claude', 'references'].includes(e)) continue;
    const full = join(p, e);
