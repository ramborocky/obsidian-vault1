---
tags: [personal, career, accounting, nbaa, todo, pinned]
date: 2026-07-22
status: active
source: nbaa.go.tz
---

# 🎓 BECOMING ACPA(T) — NBAA TANZANIA

## What ACPA Is
**Associate Certified Public Accountant (ACPA)** — the professional membership grade issued by NBAA (National Board of Accountants and Auditors, Tanzania) that a **Graduate Accountant (GA)** upgrades into after gaining practical experience. There's also **ACPA-PP** (Public Practice variant, for auditors).

*(If you're not already a registered Graduate Accountant with NBAA, that's the actual first step — everything below assumes GA status as the starting point. Flag it if that's not where you are yet and I'll dig into the GA registration route instead.)*

## Official Requirements (confirmed from nbaa.go.tz)
1. **3 years of practical work experience** as a Graduate Accountant, logged.
2. **Three completed logbooks** — one per year of practical experience — submitted as proof.
3. **Application form** (downloadable from nbaa.go.tz) filled and submitted.
4. **Supporting documents required:**
   - Copy of relevant Board-issued certificates
   - Name and address of employer
   - Two referees — professional accountants already registered with NBAA (ACPA or FCPA)
   - (For ACPA-PP specifically: referees must be ACPA-PP or FCPA-PP)
   - Two recent passport-size photographs
   - Proof of payment of the non-refundable application fee
5. Submission now goes through **MEMS** (NBAA's Members and Examination Management System) — online.
6. Application reviewed and approved by the Board.

## Progress
- **2026-08-10 — MEMS profile updated, a certificate uploaded to the professional section.** First movement on this file since 22 July. Day 2 is done; Day 6 is part-done.

## One-Task-A-Day To-Do
| Day | Task                                                                                                                                       | Status |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| 1   | Confirm your current status — **now readable off the MEMS profile, no phone call needed.** See the three fields below                      | 🔄      |
| 2   | Log into (or create) your MEMS account on nbaa.go.tz                                                                                       | ✅      |
| 3   | Download the logbook template(s) and the ACPA application form                                                                             | ⬜      |
| 4   | Fill in / update your practical-experience logbook(s) for whatever years are outstanding                                                   | ⬜      |
| 5   | Identify and contact two ACPA/FCPA-registered accountants to act as referees                                                               | ⬜      |
| 6   | Gather documents: Board certificates copy, employer details, 2 passport photos — **certificate uploaded 10 Aug; photos and employer details outstanding** | 🔄      |
| 7   | Pay the application fee and submit the full application via MEMS                                                                           | ⬜      |

### Day 1, while you are still logged in — three fields to read off the profile
The status question was going to cost a phone call. It doesn't any more; MEMS holds the answer.

1. **Membership grade** — does it say *Graduate Accountant* / show a GA registration number, or something else?
2. **Registration date** — this starts the three-year clock. Years elapsed is the number [[00 The Board]] and [[Home]] are both waiting on
3. **Practical experience / logbook section** — does one exist on the profile, and are any years already recorded in it?

**Write the three answers into the Progress line above.** That closes Home's blocker #1 and unblocks six tasks.

## Open Questions (need your input, not guessable)
- Are you already registered as a Graduate Accountant with NBAA?
- How many years of practical experience do you already have logged, if any?
- Do you want the standard **ACPA** track or **ACPA-PP** (public practice/audit)?

## Sources
- [NBAA — Associate Certified Public Accountant (ACPA) / ACPA-PP](https://www.nbaa.go.tz/pages/associate-certified-public-accountant-acpa-acpa-pp)
- [NBAA — Temporary ACPA (TACPA), non-citizens only](https://www.nbaa.go.tz/pages/temporary-associate-certified-public-accountant-tacpa)
- Contact: info@nbaa.go.tz | +255 22 2 211890
