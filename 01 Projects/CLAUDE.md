...
- **Git — verified 2026-07-29, supersedes the earlier "no remote" note.** The remote *is* connected: `origin` → `https://github.com/ramborocky/obsidian-vault1`, branch `main` tracking `origin/main`, in sync at the time of checking. Commit `f4286c7` reconnected it. Normal commits and pushes are fine; no force-push is ever needed and none should be used.
	- ⚠️ **The remote is PUBLIC.** Anything pushed is readable by anyone on the internet, including full story prose. Chantal Draft 2, the Found Things drafts and `Claude_s Version of Kalemie (AI Draft, Not Canon).md` were already public before 2026-07-29; Chantal Draft 3 was pushed on the user's explicit instruction after the risk was raised.
	- **Why it matters:** Commonwealth entry rule 3f requires a story to be unpublished, and defines published to include *published online*, excepting only personal blogs, personal websites and personal Facebook pages. Whether a public code host qualifies is a ruling only the Foundation can give (rule 2e — the chair's ruling is final). See `Kalemie/10 Prize Campaign/12 The Rules — Source of Truth.md`. **Raise this before pushing new story prose; it is the user's call, and they have made it once already.**
	- Generated submission binaries (`10 Prize Campaign/11 Submission Pack/*.docx`, `*.pdf`) are gitignored — rebuildable from the markdown, and story prose should not go to a public remote as an opaque binary.
- **Authorship matters here and is not decoration.** Rule 3e: *the story must be the entrant's own work.* In June 2026 the Foundation investigated AI-use allegations by examining working drafts, time-stamped documents and notes. Git history is therefore evidence. **Do not add a `Co-Authored-By: Claude` trailer to commits that merely track story prose Claude did not write** — it would fabricate an authorship record against the user's interest. Use it on notes, instruments and tooling Claude did author.
- **Hard submission constraint**: the Commonwealth Short Story Prize accepts 2,000–5,000 words. Chantal Draft 3 measures ~3,896 words. Re-measure before any expansion — see `Kalemie/Learning Curve/Kalemie — Standing Diagnosis.md` for the running word budget.
