...
- **Git — verified 2026-07-29, supersedes the earlier "no remote" note.** The remote *is* connected: `origin` → `https://github.com/ramborocky/obsidian-vault1`, branch `main` tracking `origin/main`, in sync at the time of checking. Commit `f4286c7` reconnected it. Normal commits and pushes are fine; no force-push is ever needed and none should be used.
	...
	- **Ask before pushing new story prose; it is the user's call, and they have made it once already.** Ask it as that one question — do not attach prize-rule reasoning to it. The background is in `Kalemie/10 Prize Campaign/12 The Rules — Source of Truth.md` if it is ever needed.
...
- **Authorship. The user writes the prose; Claude does not.** Settled — do not re-explain it or cite prize clauses as backing for it. **Do not add a `Co-Authored-By: Claude` trailer to commits that merely track story prose Claude did not write** — it would fabricate an authorship record against the user's interest. Use it on notes, instruments and tooling Claude did author.
